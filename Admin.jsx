...
const S = {
  ...
  statCard: (color) => ({ background:"white", borderRadius:12, padding:"20px 16px",
  ...
  btn: (bg) => ({ padding:"4px 12px", background:bg, color:"white",
    border:"none", borderRadius:6, fontSize:12, cursor:"pointer", fontWeight:600 }),
  section: { background:"white", borderRadius:12, padding:24, marginBottom:20,
...
export default function Admin({ navigate, logout, user }) {
  ...
  if (!stats) return <div style={{padding:40,textAlign:"center"}}>Loading admin panel...</div>
        ...
        <div style={S.section}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
            <div>
              ...
              <input style={S.input} type="number" value={giftDays}
                     onChange={e=>setGiftDays(+e.target.value)} min={1} max={3650} style={{...S.input,width:80}} />
            </div>
            <div>
              <label style={{fontSize:13,color:"#666",marginRight:6}}>Count:</label>
              <input style={S.input} type="number" value={giftCount}
                     onChange={e=>setGiftCount(+e.target.value)} min={1} max={50} style={{...S.input,width:60}} />
            </div>
          ...
          {newGifts.length > 0 && (
            <div style={{marginTop:16,padding:12,background:"#F3E5F5",borderRadius:8}}>
              ...
              {newGifts.map(code => (
                <div key={code} style={{fontFamily:"monospace",fontSize:15,fontWeight:700,
                                        color:"#1A237E",padding:"4px 0"}}>{code}</div>
              ))}
            </div>
          )}
        </div>
                ...
                <tr key={u.id}>
                  ...
                  <td style={S.td}>
                    <div style={{display:"flex",gap:6"}}>
                      ...
                      <button style={S.btn(u.is_active?"#C62828":"#2E7D32")}
